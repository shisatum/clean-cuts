...
static func voxel_center(idx: int, dims: Vector3i, body_size: Vector3) -> Vector3:
	var cell := Vector3(body_size.x / dims.x, body_size.y / dims.y, body_size.z / dims.z)
	var vx: int = idx % dims.x
	var vy: int = floori(float(idx) / dims.x) % dims.y
	var vz: int = floori(float(idx) / (dims.x * dims.y))
	return Vector3(-body_size.x * 0.5, -body_size.y * 0.5, -body_size.z * 0.5) \
		+ Vector3((vx + 0.5) * cell.x, (vy + 0.5) * cell.y, (vz + 0.5) * cell.z)

# Carved (hole) voxels that sit on the fracture interface between islands a and
# b: a void voxel with BOTH island labels somewhere in its radius-2 neighbourhood
# — i.e. wedged in the slot the holes cut between the two pieces. Fitting a plane
# to these recovers the TRUE cut orientation; the centroid-difference vector is
# the wrong axis for any non-axis-aligned cut.
static func interface_points(labels: PackedInt32Array, dims: Vector3i,
		body_size: Vector3, a: int, b: int) -> PackedVector3Array:
	var pts := PackedVector3Array()
	for idx: int in range(labels.size()):
		if labels[idx] != -1:
			continue
		if _has_both_within(labels, dims, idx, a, b, 2):
			pts.append(voxel_center(idx, dims, body_size))
	return pts

static func _has_both_within(labels: PackedInt32Array, dims: Vector3i,
		idx: int, a: int, b: int, radius: int) -> bool:
	var x: int = idx % dims.x
	var y: int = floori(float(idx) / dims.x) % dims.y
	var z: int = floori(float(idx) / (dims.x * dims.y))
	var fa := false
	var fb := false
	for dz: int in range(-radius, radius + 1):
		var nz: int = z + dz
		if nz < 0 or nz >= dims.z:
			continue
		for dy: int in range(-radius, radius + 1):
			var ny: int = y + dy
			if ny < 0 or ny >= dims.y:
				continue
			for dx: int in range(-radius, radius + 1):
				var nx: int = x + dx
				if nx < 0 or nx >= dims.x:
					continue
				var l: int = labels[nx + ny * dims.x + nz * dims.x * dims.y]
				if l == a: fa = true
				elif l == b: fb = true
				if fa and fb:
					return true
...
# Primary axis (largest-eigenvalue eigenvector of the covariance) of a point
# cloud — the direction it is most elongated along. For a fracture interface
# this is the cut LINE direction. Found by power-iterating on the covariance.
# Returns a unit vector, or ZERO if the cloud is point-like.
static func primary_axis(points: PackedVector3Array) -> Vector3:
	var c := Vector3.ZERO
	...
	for _k: int in range(48):
		var nv: Vector3 = m * v
		var ln: float = nv.length()
		if ln < 1e-12:
			break
		v = nv / ln
	return v

# Returns Vector2(min, max) of an island's voxel-centre projections onto n.
static func island_proj_range(island: PackedInt32Array, dims: Vector3i,
		body_size: Vector3, n: Vector3) -> Vector2:
	var mn: float =  INF
	var mx: float = -INF
	for raw: int in island:
		var p: float = voxel_center(raw, dims, body_size).dot(n)
		mn = minf(mn, p)
		mx = maxf(mx, p)
