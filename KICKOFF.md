...
- M4 ✅ Voxel flood-fill severs. Voxels decide WHERE to cut; each fragment renders as a **smooth** solid: a single AABB-sized `CSGBox3D` minus its hole cylinders minus one oriented clip-plane per neighbouring island (no voxel-box staircase). Recursive.
- Fracture-face polish ✅ (docs option 3) Cut faces are flat CSG half-spaces, not voxel stairsteps. Clip-plane **orientation** is fitted from the carved interface voxels (`primary_axis` → cut line → normal ⟂ to it), so angled cuts sever on the correct axis; pairwise complementary planes mean no fragment overlap / ghost geometry for any island count. Verified visually via `tests/visual_capture.tscn`.
...
- **Headless tests:** `tests/test_raycast.gd` verifies Area3D/raycast architecture; `tests/test_enemy_spawn.gd` verifies EnemySpawner places an enemy on the floor; `tests/test_clip_plane.gd` and `tests/test_split_fragments.gd` cover the fracture-plane math and smooth-fragment structure (incl. a diagonal-cut "wrong axis" guard). Run with `Godot_v4.6.3-stable_win64_console.exe --headless --path <proj> res://tests/<scene>.tscn`. **CSG does not render headless** — use `tests/visual_capture.tscn` WITHOUT `--headless` to eyeball fragment geometry (writes `tests/capture*.png`).
