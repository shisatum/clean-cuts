...
- **M5 — Enemies. 🔧 in progress**
  ...
  - **Sleep→trimesh physics swap (Red Faction pattern):** when Jolt puts a body to sleep, `sleeping_state_changed` fires → `_on_sleeping_state_changed` swaps `_phys_col.shape` to the `ConcavePolygonShape3D` already held by `_ray_col`, then sets `freeze_mode = FREEZE_MODE_STATIC` + `freeze = true`. Jolt treats the body as a static body, so the concave trimesh is valid — objects resting on a carved fragment collide with its actual surface geometry. On the next `apply_hole()` call, `_restore_box_collision()` swaps back to `BoxShape3D` and unfreezes before processing the new damage. Both swaps emit `collision_rebuilt` so the debug overlay updates automatically. **Future alternative:** if the trimesh/freeze complexity becomes a burden, the swap can be removed entirely and replaced with compound boxes for all states — the raycast Area3D (layer 2) already handles shooting through holes independently, so physics collision accuracy would fall back to voxel-resolution boxes only.
  - **Compound physics boxes (awake state — implemented):** fragments use a compound of the greedy boxes from `VoxelConnectivity.decompose_island` (one `CollisionShape3D` per greedy box) as their physics shape while dynamic. Applied at spawn time via `_apply_compound_boxes()`, stored in `_compound_boxes`. `_on_sleeping_state_changed` collapses them to a single trimesh+freeze; `_restore_box_collision` rebuilds the compound on wake. Scene-placed bodies (planks, enemies) have no compound data and fall back to a single `body_size` box.
  - **Floating-fragment fix (implemented):** when a fragment severs and its body is about to `queue_free()`, a `PhysicsShapeQueryParameters3D` query (slightly expanded AABB, layer 1) finds all sleeping `RigidBody3D` bodies nearby and wakes them (`sleeping = false`). Jolt does not automatically wake sleeping/frozen bodies when a static support body disappears — this query bridges that gap.
