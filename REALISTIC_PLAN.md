...
- **Fracture Face Aesthetics — ✅ DONE (Option 3, "plane-split for the visual face")**
  Implemented in `destructible_body.gd`. Each fragment is a single AABB-sized solid `CSGBox3D`, minus its hole cylinders (from `_hole_records`), minus one oriented `CSGBox3D` SUBTRACTION clip-plane per neighbouring island. Cut faces are flat half-spaces — no voxel staircase. Key implementation notes for anyone touching this:
  - **Clip on PRIMITIVES, never a baked mesh.** Godot CSG booleans need manifold input; clipping the baked (non-manifold) ArrayMesh silently produced ghost geometry. Box + cylinders + clip-box are all clean primitives.
  - **One plane PER pair of islands.** A single plane aimed at the average of all others degenerates for 3+ islands (interior pieces fall back to full AABB → overlap). Pairwise complementary planes tile with zero overlap for any island count. Planes that fall entirely beyond a fragment's AABB are pruned.
  - **Normal is FITTED, not the centroid difference.** The centroid-difference vector is the wrong axis for angled cuts (a diagonal slash came out clipped horizontally — the "wrong axis" bug). `_fracture_plane()` takes the primary axis (PCA) of the carved interface voxels as the cut LINE, then the island-separation direction projected ⟂ to it. Robust even though through-hole voids are also thin along the plank's thickness (a naive smallest-eigenvector plane-fit picks the thickness axis — wrong).
  - `VoxelConnectivity.carve_halfspace` keeps each fragment's voxel grid consistent with its visual clips so re-splits stay correct.
  - Verified visually (`tests/visual_capture.tscn`, windowed) for diagonal and multi-slab cuts; structurally guarded by `tests/test_split_fragments.gd` (incl. a diagonal "wrong axis" assertion) and `tests/test_clip_plane.gd`.
  Cosmetic note: a dense line of holes still leaves a faceted seam (the individual drill scallops) alongside the flat clip face — that's the bullet damage, not a voxel artifact. Hole walls now inherit the body material (were rendering default white).
