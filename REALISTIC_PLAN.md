...
- **M4 — Procedural fracture.** Voxel flood-fill connectivity check (~900 voxels/object, auto-scaled). After each hole, BFS finds disconnected islands. Each island ≥ 8% of original volume becomes a `FragmentObject` (RigidBody3D, convex-hull collision, carries its own hole history). Fragments are fully recursive — re-shoot a fragment and it can split again. Dust (< 8%) deleted silently. Angular velocity set proportional to COM offset so longer halves spin naturally. Generalises to any box-shaped destructible.
- **Pre-M5 refactor — Unify DestructibleObject + FragmentObject.** Currently two scripts with duplicated logic held apart by GDScript's single-inheritance. Goal: one `DestructibleBody` script extending `Node3D` that builds its own `RigidBody3D` (frozen for static, unfrozen for fragments) and `CSGCombiner3D` as children at runtime. Attempted once — caused splitting regression. The missing detail to resolve before retrying: understand exactly why `cyl.position` for holes differs between the CSGCombiner3D-as-root case vs the CSGCombiner3D-as-child case, and ensure the voxel body-local-space lookup stays consistent after the hierarchy change.
