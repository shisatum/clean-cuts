...
- **M5 — Enemies. 🔧 in progress**
  ...
  - **Test infrastructure:** `tests/test_raycast.gd` is a headless smoke-test (run with `--headless`). Verifies Area3D exists on layer 2 and raycast hits it (not the physics body box). All 8 assertions green = correct architecture.
